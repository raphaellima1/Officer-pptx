...
my <- read_pptx('blank.pptx') %>% 
  ...
  ph_with(block_list(fpar(ftext(glue("COMPORTAMENTO DIÁRIO DA ARRECADAÇÃO - Adicional de 2%"), 
...
my <- my %>%
  ...
  ph_with(block_list(fpar(ftext('Adicional de 2% - TOTAL', 
...
my <- my %>%
  ...
  # Titulo e subtitulo
  ph_with(block_list(fpar(ftext(glue('Adicional de 2% - {grupo}'), 
                                prop = fp_text(font.size = 18, 
                                               color = "#2d2e2d")), 
                          fp_p = border1)),
